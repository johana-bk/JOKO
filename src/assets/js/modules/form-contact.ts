	...
	window.addEventListener("DOMContentLoaded", () => {
		...
		if (
			...
			form.querySelector("#form-name").addEventListener("keyup", function () {
				if (form.querySelector("#form-name").value.length > 2) {
					...
					if (
						form.querySelector("#form-name").value.match(/^[a-zA-Z|\s]{2,20}$/)
					) {
						i1 = 1;
						...
						form.querySelector(".error-form-name").innerHTML =
							"Le nom doit contenir entre 2 et 20 caractères et ne doit pas contenir de caractères spéciaux ou de chiffres";
					}
				}
			});
				...
				.addEventListener("keyup", function () {
					if (form.querySelector("#form-message").value.length > 20) {
						...
						if (
							form
								.querySelector("#form-message")
								.value.match(/^[a-zA-Z0-9\s\.\,\!\?\:\;\(\)\'\"]{20,300}$/)
						) {
							i4 = 1;
							...
							form.querySelector(".error-form-message").innerHTML =
								"Le message doit contenir entre 20 et 300 caractères";
						}
					}
				});
		}
		...
		form.addEventListener("submit", (e) => {
			...
			if (
				...
				parseInt(i1) + parseInt(i2) + parseInt(i3) + parseInt(i4) == 4
			) {
				...
				alert(
					"Bonjour " +
						name +
						", votre message a bien été envoyé, l'agence vous contactera prochainement",
				);
			}
		});
	});
